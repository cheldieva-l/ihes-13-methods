...
Method 1 Version 3 completed on T4 x2 but exposed a resume defect: the cancelled Version 2 source yielded zero accepted completed rows, and the run duplicated IDs 100–102. It remains a verified non-advancing failed shard, not new aggregate evidence. Resume now fails on zero rows and asserts the exact expected completed IDs before any beam search. The corrected [Version 4 plan](results/kaggle/01_v4_rerun_plan.md) pins successful Version 3, requires resumed IDs 100–102, and permits only new IDs 103–105 without reducing the requested beam width or transform portfolio.
