...
def _score_actions_numpy(
    parent_states: np.ndarray,
    scorer: Callable[[torch.Tensor], torch.Tensor],
    action_count: int,
    config: BeamConfig,
) -> np.ndarray:
    """Score every action from each parent with one multi-output evaluation.

    Scorers opt into this path by exposing ``score_actions(states)``.  The
    returned matrix must preserve parent-major/action-minor expansion order.
    """

    parts: list[np.ndarray] = []
    device_type = torch.device(config.device).type
    score_actions = getattr(scorer, "score_actions", None)
    if score_actions is None:
        raise TypeError("scorer does not expose score_actions")
    for offset in range(0, len(parent_states), config.inference_batch):
        batch = torch.from_numpy(
            parent_states[offset : offset + config.inference_batch]
        ).to(config.device)
        with torch.inference_mode(), torch.autocast(
            device_type=device_type,
            dtype=torch.float16,
            ...
            values = score_actions(batch)
        array = values.detach().float().cpu().numpy()
        if array.ndim != 2 or array.shape[1] != action_count:
            raise ValueError(
                f"score_actions returned {array.shape}; expected (*, {action_count})"
            )
        parts.append(array if config.smaller_is_better else -array)
    return (
    ...
    *,
    ...
    for depth in range(1, config.max_depth + 1):
        ...
        for parent_offset in range(0, len(current_states), config.parent_chunk):
            ...
            action_scorer = getattr(scorer, "score_actions", None)
            if action_scorer is not None:
                child_scores = _score_actions_numpy(
                    parents_chunk, scorer, puzzle.generator_count, config
                )
            if config.prune_immediate_inverse:
                ...
                if action_scorer is not None:
                    child_scores = child_scores[allowed]
            generated_count += len(child_states)
            if action_scorer is None:
                child_scores = _score_numpy(child_states, scorer, config)
            if config.cumulative_alpha:
                parent_scores = current_scores[child_parents]
                child_scores = child_scores + np.float32(config.cumulative_alpha) * parent_scores
